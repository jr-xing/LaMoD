...
def get_rot_mat_2dt(theta, device):
    # Calculate rotation matrix for the given angle
    theta = torch.tensor(theta, device=device)
    rot_mat_2d = torch.tensor([[torch.cos(theta), -torch.sin(theta), 0],
                               [torch.sin(theta), torch.cos(theta), 0],
                               [0, 0, 1]], device=device)  # 3x3 rotation matrix
    
    # Extend to a 3x4 matrix for affine transformation
    rot_mat_3d = torch.cat([rot_mat_2d, torch.tensor([[0], [0], [0]], device=device)], dim=1)
...
def rotate_displacement_field_seq(x, theta, dtype, device):
    B, C, H, W, T = x.shape
    rotated_disp_2d = rot_seq(x, theta, dtype, device)

    # Rotate the displacement vectors according to the chosen angle
    cos_theta = torch.cos(torch.tensor(theta))
    sin_theta = torch.sin(torch.tensor(theta))
    u_disp = rotated_disp_2d[:, 0, :, :]
    v_disp = rotated_disp_2d[:, 1, :, :]
