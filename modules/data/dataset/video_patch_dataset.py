...
class VideoPatchDataset(TorchDataset):
    def __init__(self, data, augmentation=None, config=None, full_config=None, dataset_name=None):
        ...
        self.img_frame_dim = config.get('img_frame_dim', -1)
        ...
        self.DENSE_disp_frame_dim = config.get('DENSE_disp_frame_dim', -1)
        ...
        if self.frame_aligning_method == 'constant':
            ...
            for datum_idx, datum in enumerate(self.data):
                n_frames = datum[self.img_key].shape[self.img_frame_dim]
    ...
    def __getitem__patchify_minimal_overlap(self, index):
        ...
        if self.disp_type == 'Eulerian':
            ...
            T = raw_tar.shape[self.img_frame_dim]
            
            # src = np.repeat(raw_src[..., 0:1], self.frame_patchify_n_frames, axis=-1)
            

        if self.return_DENSE_disp:
            raw_DENSE_disp = np.moveaxis(self.data[tar_datum_idx][self.DENSE_disp_key], self.DENSE_disp_frame_dim, 1)            
            # print(f'{raw_DENSE_disp.shape=}')
            datum['DENSE_disp'] = raw_DENSE_disp[:, tar_starting_frame_idx:tar_starting_frame_idx+self.frame_patchify_n_frames]

        # src = np.moveaxis(src, self.img_frame_dim, 0)[None, ...]
        raw_tar = np.moveaxis(raw_tar, self.img_frame_dim, 1)#[None, ...]
        # print(f'{raw_tar.shape=}')
